...
"""
Verified position player with goal re-check and retry.

Enhancements:
1. Supports absolute pose sequence files (Pose=...), reducing delta drift.
2. Supports post-action goal verification with optional retries.

Backward compatibility:
- Still supports legacy delta action lines (Action=...).

Recommended usage:
- Prefer sequence_pose.txt generated by sequence_recorder.py.
- Pose= lines are interpreted as absolute poses.
- Action= lines are interpreted by action_mode (default: delta).
"""
...
class PositionSequencePlayer(object):
    def __init__(self, robot_type='j2s6s300'):
        ...
        # -------------------------
        # File and action interpretation parameters
        # -------------------------
    ...
    # ------------------------------------------------------------------
    # Pose action execution and verification
    # ------------------------------------------------------------------
